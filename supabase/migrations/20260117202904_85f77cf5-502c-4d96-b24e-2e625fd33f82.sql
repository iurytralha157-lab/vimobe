...
DROP FUNCTION IF EXISTS public.create_default_stages_for_pipeline(uuid, uuid);

-- Recreate with fixed variable names to avoid ambiguity
...
BEGIN
  FOR i IN 1..array_length(default_stages, 1) LOOP
    -- Create stage
    INSERT INTO stages (pipeline_id, name, color, position, organization_id)
    VALUES (p_pipeline_id, default_stages[i], stage_colors[i], i, p_org_id)

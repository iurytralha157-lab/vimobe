-- Fix list_contacts_paginated - use correct permission tables
...
BEGIN
  ...
  -- Get user's organization and check super admin
  SELECT 
    u.organization_id, 
  ...
  -- Get permissions using correct table name
  SELECT 
  ...
  FROM users u
  LEFT JOIN organization_roles r ON r.id = u.role_id

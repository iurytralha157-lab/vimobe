-- Corrigir função handle_lead_intake removendo referência à coluna inexistente assigned_at
...
BEGIN
  ...
  IF v_assigned_user_id IS NOT NULL THEN
    -- Update lead (removed assigned_at - column does not exist)
    UPDATE public.leads 
    ...
    -- Create activity
    INSERT INTO public.activities (lead_id, organization_id, user_id, type, content) 
    VALUES (p_lead_id, v_lead.organization_id, v_assigned_user_id, 'assignee_changed', 'Lead atribuído via round-robin');
  ...
  RETURN jsonb_build_object('success', true, 'lead_id', p_lead_id, 'assigned_user_id', NULL, 'round_robin_used', true, 'reason', 'No available members');
END;
$$;
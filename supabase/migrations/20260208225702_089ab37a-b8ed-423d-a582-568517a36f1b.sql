...
BEGIN
  ...
  LOOP
    ...
    -- meta_form_id usando form_id do JOIN
    IF v_match ? 'meta_form_id' AND jsonb_array_length(v_match->'meta_form_id') > 0 THEN
      IF v_lead.form_id IS NULL OR NOT (v_lead.form_id = ANY(ARRAY(SELECT jsonb_array_elements_text(v_match->'meta_form_id')))) THEN
        CONTINUE;
      END IF;
    END IF;

-- Migration para adicionar validação de escala na distribuição de leads (handle_lead_intake)
...
BEGIN
  ...
  -- Selecionar próximo usuário validando escala
  -- Verifica se o corretor pertence a uma rrm.team_id, junta c/ team_members e valida member_availability
  -- Caso o RRM use user_id direto (sem team), o corretor precisa estar em alguma team e ter availability vinculada.
  -- Por segurança, buscamos a disponibilidade baseando no user_id batendo c/ team_members.user_id na disponibilidade.
  ...
  WHERE rrm.round_robin_id = v_queue.id
    ...
    AND EXISTS (
      -- Verifica se o membro tem escala ativa no momento da distribuição
      SELECT 1 
      FROM team_members tm
      JOIN member_availability ma ON ma.team_member_id = tm.id
      WHERE tm.user_id = u.id
        AND ma.day_of_week = v_current_day
        AND ma.is_active = true
        AND (
          ma.is_all_day = true OR 
          (v_current_time >= ma.start_time::TIME AND v_current_time <= ma.end_time::TIME)
        )
    )
  ORDER BY rrm.leads_count ASC NULLS FIRST, rrm.position ASC
  LIMIT 1;
  
  -- FALLBACK 2: Se fila não tem membros ativos ou NENHUM NA ESCALA
  IF v_next_user_id IS NULL THEN
    ...
    IF v_admin_id IS NOT NULL THEN
      ...
      INSERT INTO lead_timeline_events (
        ...
        p_lead_id, v_org_id, v_admin_id, 'lead_assigned',
        'Atribuído ao administrador',
        'Fila "' || v_queue.name || '" sem membros disponíveis na escala atual. Atribuído a ' || COALESCE(v_next_user_name, 'administrador'),
        jsonb_build_object(
          'destination', 'admin_fallback',
          'queue_name', v_queue.name,
          'queue_id', v_queue.id,
          'reason', 'no_active_or_available_members',
          'assigned_user_id', v_admin_id,
          'assigned_user_name', v_next_user_name
        )
      );
    ...
    ELSE
      INSERT INTO lead_timeline_events (
        ...
        p_lead_id, v_org_id, NULL, 'lead_assigned',
        'Enviado para o Pool',
        'Fila "' || v_queue.name || '" sem membros disponíveis, e nenhum admin encontrado',
        jsonb_build_object(
          'destination', 'pool',
          'queue_name', v_queue.name,
          'queue_id', v_queue.id,
          'reason', 'no_active_or_available_members_no_admin'
        )
      );
    END IF;
    RETURN;
  ...
  INSERT INTO lead_timeline_events (
    ...
    'Atribuído a ' || COALESCE(v_next_user_name, 'usuário') || ' pela fila "' || v_queue.name || '" dentro do horário da escala.',
    jsonb_build_object(
      'source', v_lead.source,
      'queue_name', v_queue.name,
      'queue_id', v_queue.id,

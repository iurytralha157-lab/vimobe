-- Re-enable the trigger and fix handle_lead_intake to use correct activities columns
ALTER TABLE public.leads ENABLE TRIGGER trigger_lead_intake;

-- Fix handle_lead_intake function - activities table doesn't have organization_id
...
BEGIN
  ...
  IF v_assigned_user_id IS NOT NULL THEN
    ...
    -- Create activity - FIXED: activities table only has (lead_id, user_id, type, content, metadata)
    INSERT INTO public.activities (lead_id, user_id, type, content) 
    VALUES (p_lead_id, v_assigned_user_id, 'assignment', 'Lead atribuído automaticamente via round-robin');

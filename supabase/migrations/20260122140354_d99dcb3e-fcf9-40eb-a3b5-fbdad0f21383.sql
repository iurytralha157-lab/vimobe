-- Drop and recreate handle_lead_intake function with CORRECT database structure
...
BEGIN
  ...
  -- 2. Find active round-robin queue for this organization (NOT pipeline)
  SELECT * INTO v_queue
  FROM round_robins
  ...
  IF NOT FOUND THEN
    -- Fallback to first member if offset fails
    SELECT * INTO v_member
    FROM round_robin_members
    WHERE round_robin_id = v_queue.id
    ORDER BY position ASC
  ...
  -- 9. Log activity (activities table does NOT have organization_id)
  ...
  -- 10. Log to round_robin_logs (this table HAS organization_id)

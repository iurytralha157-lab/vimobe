-- Drop existing functions to recreate with parameters
DROP FUNCTION IF EXISTS public.get_funnel_data();
DROP FUNCTION IF EXISTS public.get_lead_sources_data();

-- Recreate get_funnel_data with filter parameters
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(s.key, s.name) AS stage_key,
  ...
  GROUP BY s.id, s.name, s.key, s.order_index
  ORDER BY s.order_index;
END;
$$;

-- Recreate get_lead_sources_data with filter parameters

...
export function exportContracts({ contracts, filename = 'contratos', exportFormat = 'xlsx' }: ExportOptions) {
  const exportData = contracts.map(contract => ({
    ...
    'CPF/CNPJ': contract.client_document || '',
    ...
    'Condições de Pagamento': contract.payment_conditions || '',
    'Data Início': formatDateSafe(contract.start_date),
    'Data Fim': formatDateSafe(contract.end_date),

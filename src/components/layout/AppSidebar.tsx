          ...
          {navItems.map(item => (
            <li key={item.path}>
              {item.children && !collapsed ? (
                <Collapsible 
                  open={isMenuOpen(item.path) || isActiveParent(item)} 
                ...
                >
                  <CollapsibleTrigger asChild>
                    <button className={cn(
                      ...
                      isActiveParent(item) && "text-sidebar-foreground bg-orange-100 dark:bg-orange-900/30"
                    )}>
                      <item.icon className="h-5 w-5 flex-shrink-0" />
                      <span className="flex-1 text-left">{getLabel(item.labelKey)}</span>
                      <ChevronDown className={cn(
                        "h-4 w-4 transition-transform",
                        (isMenuOpen(item.path) || isActiveParent(item)) && "rotate-180"
                      )} />
                    </button>
                  </CollapsibleTrigger>
                    ...
                    {item.children.map(child => (
                      <NavLink
                        ...
                        className={cn(
                          ...
                          "text-sidebar-foreground/60 hover:text-sidebar-foreground hover:bg-orange-100 dark:hover:bg-orange-900/30",
                          location.pathname === child.path && "text-sidebar-foreground bg-orange-100 dark:bg-orange-900/30"
                        )}
                      >

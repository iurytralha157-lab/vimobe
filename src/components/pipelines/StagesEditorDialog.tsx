...
export function StagesEditorDialog({
  ...
  const handleSave = async () => {
    const pipelineNameChanged = currentPipelineName !== pipelineName;
    if (!hasChanges && !pipelineNameChanged) {
      onOpenChange(false);
      return;
    }
    ...
    try {
      ...
      if (hasChanges) {
        ...
        const stagesToUpsert = stages.map((stage, index) => {
          const stageKey = stage.stage_key || stage.name
            .toLowerCase()
            .normalize('NFD')
            .replace(/[\u0300-\u036f]/g, '')
            .replace(/[^a-z0-9]/g, '_');
      ...
      toast.success(pipelineNameChanged ? 'Pipeline e colunas atualizados!' : 'Colunas atualizadas com sucesso!');
      onStagesUpdated();
      onOpenChange(false);
    } catch (error: any) {
